...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // Apply pill state immediately (before async overlay open) so it renders correctly on load
  if (shouldOpenV1) {
    updateV1Toggle(true);
  }
  // Force browser to paint the initial pill position BEFORE any transitions are triggered.
  // Without this reflow, the pill has no "from" state and the first-click animation is skipped.
  document.querySelectorAll(".nav-v1-toggle").forEach(function(el) {
    // Reading offsetWidth forces a synchronous layout/paint
    void el.offsetWidth;
  });
