...
async function getFightTitles() {
  ...
  const data = await fetchJSON(DATA_BASE + "/titles/fight_titles.json?cb=" + Date.now());
  if (data && (data.upcoming || data.upcoming_event)) return data;
  // Fallback: V1 repo titles
  const v1data = await fetchJSON(V1_DATA_BASE + "/titles/fight_titles.json?cb=" + Date.now());
  if (v1data && (v1data.upcoming || v1data.upcoming_event || v1data.recent)) return v1data;
  // Last resort: return empty but non-null so callers get a clean {}
  return {};
}
...
async function getV2Predictions() {
  ...
  if (!clean) return [];
  return await fetchCSV(DATA_BASE + "/predictions/v2_betting_recommendations_" + clean + ".csv?cb=" + Date.now());
}
...
async function getV2PropPredictions() {
  ...
  const clean  = cleanEventName(titles.upcoming || "");
  if (!clean) return [];
  return await fetchCSV(DATA_BASE + "/predictions/v2_prop_recommendations_" + clean + ".csv?cb=" + Date.now());
}
...
async function getV1Predictions() {
  ...
  if (!clean) return [];
  const a = await fetchCSV(V1_DATA_BASE + "/predictions/v1_predictions_latest.csv?cb=" + Date.now());
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // Only open V1 if explicitly requested via ?model=v1 URL param.
  // Never restore from localStorage — always default to V2 on fresh page load.
  const urlParam = new URLSearchParams(window.location.search).get("model");
  const shouldOpenV1 = urlParam === "v1";
  // Apply pill state immediately - using transform approach, this reliably animates on first click
  if (shouldOpenV1) {
    // Disable transition momentarily so pill snaps to V1 position instantly on page load
    document.querySelectorAll(".nav-v1-toggle").forEach(function(el) {
      el.style.setProperty("--pill-no-transition", "none");
    });
    updateV1Toggle(true);
    // Re-enable transition after a frame so subsequent clicks animate
    requestAnimationFrame(function() {
      requestAnimationFrame(function() {
        document.querySelectorAll(".nav-v1-toggle").forEach(function(el) {
          el.style.removeProperty("--pill-no-transition");
        });
      });
    });
  }
...
// System theme changes ignored — default is always dark
