...
function openV1Overlay() {
  ...
  if (!overlay) return;
  // Instant swap - hide main, show V1. No scrolling. Nav stays identical.
  overlay.style.display = "block";
  if (mainEl) mainEl.style.display = "none";
  if (footerEl) footerEl.style.display = "none";
  // Scroll page to top so V1 starts at top of viewport
  window.scrollTo(0, 0);
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // V1 toggle - use a shared open/closed flag to avoid double-fire from desktop + mobile toggles
  let _v1Open = false;
  document.querySelectorAll(".nav-v1-toggle").forEach(function(el) {
    el.addEventListener("click", function() {
      if (_v1Open) { _v1Open = false; closeV1Overlay(); }
      else         { _v1Open = true;  openV1Overlay();  }
    });
  });
